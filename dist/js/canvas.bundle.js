...
var Player = /*#__PURE__*/function () {
  ...
  _createClass(Player, [{
    ...
    value: function update() {
      ...
      if (this.position.y + this.velocity.y + this.height <= canvas.height) this.velocity.y += gravity;
    }
  }]);
...
var Platform = /*#__PURE__*/function () {
  function Platform(_ref) {
    ...
    this.width = image.width; //this.width = 80
...
var platformSmallTallImage = createImage(_img_platformSmallTall_png__WEBPACK_IMPORTED_MODULE_3__["default"]); //creating objects for game

var player = new Player();
var platforms = [];
var genericObjects = []; //variables
...
function animate() {
  ...
  player.update();
  if (keys.right.pressed && player.position.x < 450) player.velocity.x = player.speed;else if (keys.left.pressed && player.position.x > 100 || keys.left.pressed && scrollOffSet == 0 && player.position.x > 0) player.velocity.x = -1 * player.speed;else {
    player.velocity.x = 0;
  ...
  if (player.position.y > canvas.height) init(); //win condition 
